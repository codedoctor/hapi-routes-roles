(function() {
  ...
  module.exports = function(plugin, options) {
    var fnAccountId, fnRaise404, fnRolesBaseUrl, hapiIdentityStore, methodsRoles;
    if (options == null) {
      options = {};
    }
    ...
    fnRolesBaseUrl = function() {
      return "" + options.baseUrl + options.routesBasePath;
    };
    plugin.route({
      ...
      handler: function(request, reply) {
        return fnAccountId(request, function(err, accountId) {
           ...
           */
          return methodsRoles().all(accountId, null, function(err, rolesResult) {
            var baseUrl;
            if (err) {
              return reply(err);
            }
